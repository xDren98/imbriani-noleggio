...
function saveCF(cf) { localStorage.setItem('cf', cf); }
function loadCF() { return localStorage.getItem('cf') || ''; }
...
function renderDrivers() {
  ...
  cont.innerHTML = '';
  DRIVERS.forEach((d, i) => cont.insertAdjacentHTML('beforeend', driverRow(i, d)));
}
...
async function doLogin() {
  ...
  if (!validateCF(cf)) return toast('CF non valido (16 caratteri A-Z/0-9)');
  showSpinner(true);
  try {
    const res = await callAPI({ action: 'login', token: APP_CONFIG.AUTH_TOKEN, cf });
    if (res.success) {
      CURRENT_CF = cf; saveCF(cf);
      document.getElementById('login-section').classList.add('d-none');
      document.getElementById('area-personale').classList.remove('d-none');
      await loadBookings();
      await loadAvailability();
    } else toast(res.message || 'Errore login');
  } catch (e) { toast('Errore rete'); }
  finally { showSpinner(false); }
}

async function loadBookings() {
  const res = await callAPI({ action: 'recuperaPrenotazioni', token: APP_CONFIG.AUTH_TOKEN, cf: CURRENT_CF });
  if (res.success) renderBookings(res.data || []);
}

async function loadAvailability() {
  // semplice: carica tutti i veicoli e lascia filtro 9 posti + Disponibile
  const res = await callAPI({ action: 'disponibilita', token: APP_CONFIG.AUTH_TOKEN, dataInizio: '', dataFine: '' });
  if (res.success) renderVehicles(res.data || []);
}
...
async function createBooking() {
  ...
  const o2 = document.getElementById('ora-consegna').value;
  const dest = document.getElementById('destinazione').value;
  if (!d1 || !d2 || !dest) return toast('Compila date e destinazione');
  if (!SELECTED_VEHICLE) return toast('Seleziona un veicolo');

  const payload = {
    action: 'creaPrenotazione', token: APP_CONFIG.AUTH_TOKEN, cf: CURRENT_CF,
    dataRitiro: d1, oraRitiro: o1, dataConsegna: d2, oraConsegna: o2, targa: SELECTED_VEHICLE.Targa,
    destinazione: dest, numAutisti: DRIVERS.length.toString(), drivers: encodeURIComponent(JSON.stringify(DRIVERS))
  };
  showSpinner(true);
  try {
    const res = await callAPI(payload);
    if (res.success) { toast('Prenotazione inviata!'); await loadBookings(); }
    else toast(res.message || 'Errore creazione');
  } catch (e) { toast('Errore rete'); }
  finally { showSpinner(false); }
}
...
function init() {
  const saved = loadCF();
  if (validateCF(saved)) { document.getElementById('cf-input').value = saved; }
  document.getElementById('login-btn').onclick = doLogin;
  document.getElementById('add-driver').onclick = () => {
    if (DRIVERS.length >= 3) return toast('Max 3 autisti');
    if (DRIVERS.length === 0) DRIVERS.push({}); else DRIVERS.push({});
